...
one_hot_instance = OneHotEncoder(
    dataset=df_data,
    column_sequence=name_column,
    max_length=50
)

data_coded = one_hot_instance.run_process()
data_coded[column_with_response] = df_data[column_with_response].values
print(data_coded.head(5))

data_coded.to_csv(name_output, index=False)