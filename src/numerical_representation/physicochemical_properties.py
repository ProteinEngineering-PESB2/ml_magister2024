...
column_with_response = sys.argv[4]

df_data["length"] = df_data[name_column].str.len()
df_data = df_data[df_data['length']<=50]
df_data.reset_index(inplace=True)

dataset_encoder=pd.read_csv(sys.argv[5])
dataset_encoder.index = dataset_encoder["residue"]
dataset_encoder.head(5)

physicochemical_instance = PhysicochemicalEncoder(
    dataset=df_data,
    property_encoder=sys.argv[6],
    dataset_encoder=dataset_encoder,
    name_column_seq=name_column,
    columns_to_ignore=[column_with_response]
)

physicochemical_instance.run_process()
print(physicochemical_instance.df_data_encoded.head(5))

physicochemical_instance.df_data_encoded.to_csv(name_output, index=False)
